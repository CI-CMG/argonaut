...
import edu.colorado.cires.argonaut.core.netcdf.v31.ArgoProfileV31Parameter;
import edu.colorado.cires.argonaut.core.netcdf.v31.ArgoProfileDirection;
import edu.colorado.cires.argonaut.core.netcdf.v31.ArgoProfileDataMode;
...
public class NetCdfTiedArgoProfileV31 implements ArgoProfileV31 {
  ...
  NetCdfTiedArgoProfileV31(NetCdfTiedArgoMultiProfileV31 parent, int profileIndex) {
    ...
    try {
      platformNumber = NetCdfUtils.getProfileString(netcdf, profileIndex, "PLATFORM_NUMBER");
      projectName = NetCdfUtils.getProfileString(netcdf, profileIndex, "PROJECT_NAME");
      principalInvestigatorName = NetCdfUtils.getProfileString(netcdf, profileIndex, "PI_NAME");
      stationParameters = NetCdfUtils.getProfileListOfString(netcdf, profileIndex, "STATION_PARAMETERS");
      cycleNumber = NetCdfUtils.getProfileInt(netcdf, profileIndex, "CYCLE_NUMBER");
      direction = ArgoProfileDirection.valueOf(NetCdfUtils.getProfileString(netcdf, profileIndex, "DIRECTION"));
      dataCenter = NetCdfUtils.getProfileString(netcdf, profileIndex, "DATA_CENTRE");
      dataCenterReference = NetCdfUtils.getProfileString(netcdf, profileIndex, "DC_REFERENCE");
      dataStateIndicator = NetCdfUtils.getProfileString(netcdf, profileIndex, "DATA_STATE_INDICATOR");
      dataMode = ArgoProfileDataMode.valueOf(NetCdfUtils.getProfileString(netcdf, profileIndex, "DATA_MODE"));
      platformType = NetCdfUtils.getProfileString(netcdf, profileIndex, "PLATFORM_TYPE");
      floatSerialNumber = NetCdfUtils.getProfileString(netcdf, profileIndex, "FLOAT_SERIAL_NO");
      firmwareVersion = NetCdfUtils.getProfileString(netcdf, profileIndex, "FIRMWARE_VERSION");
      wmoInstrumentType = NetCdfUtils.getProfileString(netcdf, profileIndex, "WMO_INST_TYPE");
      julianDate = calculateJulianDate(netcdf, profileIndex, parent.getReferenceDateTime(), "JULD");
      julianDateQc = NetCdfUtils.getProfileString(netcdf, profileIndex, "JULD_QC");
      julianDateOfLocation = calculateJulianDate(netcdf, profileIndex, parent.getReferenceDateTime(), "JULD_LOCATION");
      latitude = NetCdfUtils.getProfileDouble(netcdf, profileIndex, "LATITUDE");
      longitude = NetCdfUtils.getProfileDouble(netcdf, profileIndex, "LONGITUDE");
      positionQc = NetCdfUtils.getProfileString(netcdf, profileIndex, "POSITION_QC");
      positioningSystem = NetCdfUtils.getProfileString(netcdf, profileIndex, "POSITIONING_SYSTEM");
      positionErrorReported = NetCdfUtils.getProfileFloat(netcdf, profileIndex, "POSITION_ERROR_REPORTED");
      positionErrorEstimatedComment = NetCdfUtils.getProfileString(netcdf, profileIndex, "POSITION_ERROR_ESTIMATED_COMMENT");
      verticalSamplingScheme = NetCdfUtils.getProfileString(netcdf, profileIndex, "VERTICAL_SAMPLING_SCHEME");
      configMissionNumber = NetCdfUtils.getProfileInt(netcdf, profileIndex, "CONFIG_MISSION_NUMBER");
      numHistory = NetCdfUtils.getDimensionSize(netcdf, "N_HISTORY");
    } catch (IOException e) {
      throw new RuntimeException("Unable to initialize profile data wrapper", e);
    }
  }

  private static Instant calculateJulianDate(NetcdfFile netcdf, int profileIndex, Instant referenceDateTime, String variable) throws IOException {
    Double daysSinceRef = NetCdfUtils.getProfileDouble(netcdf, profileIndex, variable);
